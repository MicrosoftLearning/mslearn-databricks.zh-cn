...
1. 在边栏中，使用“**(+)新建**”链接来创建**笔记本**，并将默认笔记本名称（**无标题笔记本*[日期]***）更改为**数据处理**。 然后，在“**连接**”下拉列表中，选择群集（如果尚未选择）。 如果群集未运行，可能需要一分钟左右才能启动。
...
1. 使用以下代码替换第一个笔记本中的代码。 然后运行它，将数据加载到数据帧中：
...
1. 在现有代码单元格下移动鼠标，然后使用出现的 **+代码**图标添加新的代码单元格。 然后在新单元格中，输入并运行以下代码，按产品类别汇总销售数据：
...
1. 将默认作业名称 (**New job *[date]***) 更改为 `Automated job`。
...
1. 选择“**立即运行**”
    ...
    **提示**：在右侧面板的“**计划**”中，可以选择“**添加触发器**”并设置运行作业的计划（例如每日、每周）。 但是，对于本练习，我们将手动执行它。

1. 在“作业”面板中选择“运行”**** 选项卡并监控作业运行。

1. 作业运行成功后，可以在“**运行**”列表中选择该作业并验证其输出。
