...
1. 在左窗格的“**资源管理**”下，选择“**密钥和终结点**”。

1. 复制终结点和其中一个可用密钥，因为稍后将在本练习中使用它。
...
1. 登录到 Azure 门户，地址为 ****。
1. 请使用以下设置创建 **Azure Databricks** 资源：
...
1. 选择“**查看 + 创建**”，然后等待部署完成。 然后转到资源并启动工作区。
...
1. 在新单元格中，运行以下代码，使用检索器和 `gpt-4o` 模型创建 QA 系统：
